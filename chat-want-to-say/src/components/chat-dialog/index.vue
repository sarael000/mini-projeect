...
export default {
  ...
  data () {
    return {
      // Messages that have already been shown
      messages: [],
      // Message chains
      ...
      // Current Status: Waiting for user interaction
      status: 'systemInput',
      // The currently open component
      ...
      return new Promise((resolve) => {
        this.sendSysMsgInner(messages, inputSpeed, author).then(() => {
          if (triggerNextAction) {
            // The behavior is required to trigger the next message activity
